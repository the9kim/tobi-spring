...
@ContextConfiguration("/UserService18-Test-applicationContext.xml")
public class UserService18Test {

    public static void main(String[] args) {
        JUnitCore.main("spring.ch6_aop.l_transaction_attribute.UserService18Test");
    }

    static class TestUserService10Impl extends UserService10Impl {
        private String id = "4";

        @Override
        public void upgradeLevel(User3 user) throws SQLException {
            if (user.getId().equals(this.id)) {
                throw new UserService18Test.TestUserServiceException();
            }
            super.upgradeLevel(user);
        }

        // For transaction propagation test
        @Override
        public List<User3> getAll() {
            for (User3 user : super.getAll()) {
                super.update(user);
            }
            return null;
        }
    ...
    @Autowired
    UserService2 testUserService10Impl;
    ...
    public void upgradeAllOrNothing() throws SQLException {
        ...
        try {
            this.testUserService10Impl.upgradeLevels();
            fail("TestUserServiceException expected");
        } catch (UserService18Test.TestUserServiceException e) {
        }
