...
@ContextConfiguration("/UserService13-Test-applicationContext.xml")
public class UserService13Test {

    public static void main(String[] args) {
        JUnitCore.main("spring.ch6_aop.g_factory_bean.UserService13Test");
    }
    ...
    static class TestUserService9 extends UserService9Impl {
        ...
        public void upgradeLevel(User3 user) throws SQLException {
            if (user.getId().equals(this.id)) {
                throw new UserService13Test.TestUserServiceException();
            }
    ...
    public void upgradeAllOrNothing() throws SQLException {
        ...
        UserService9Impl target = new UserService13Test.TestUserService9(users.get(3).getId());
        target.setUserDao3(this.userDaoJdbc3);
        target.setMailSender(this.mailSender);

        TxProxyFactoryBean beanFactory = context.getBean("&userService", TxProxyFactoryBean.class);
        beanFactory.setTarget(target);
        UserService proxy = (UserService) beanFactory.getObject();
        ...
        } catch (UserService13Test.TestUserServiceException e) {
        }
